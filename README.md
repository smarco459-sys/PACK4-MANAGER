...
Instruções iniciais para configuração do workspace e associação do utilizador administrador na base de dados Supabase[cite: 3]:

```sql
-- Primeiro acesso: associar um utilizador ao workspace PACK4.
-- SUBSTITUIR os dois UUIDs pelos valores reais.
--
-- insert into public.workspace_members (workspace_id, user_id, role)
-- values ('WORKSPACE_UUID', 'AUTH_USER_UUID', 'admin')
-- on conflict (workspace_id, user_id) do update set role = excluded.role;
--
-- Não colocar chaves secretas neste ficheiro.
```[cite: 3]

---

### 3. `App.jsx`
O componente principal do frontend em React, contendo a estrutura de navegação (`nav`), o sistema de autenticação (`Login`), o contentor da aplicação (`Shell`), bem como as vistas de Dashboard, Gestão de Serviços, Kanban, Calendário, Clientes, Técnicos, Peças, Relatórios e Definições[cite: 4].
